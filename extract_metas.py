...
def carregar_google_manual(mes_ref):
    ...
    try:
        ...
        print(f"📘 Google manual (meta) carregado para {mes_ref}: {list(bloco_mes.keys())}")
        return bloco_mes
...
def montar_google_meta(bloco_google):
    """
    Só a META vem manualmente daqui. O 'ate_o_momento', 'falta' e 'progresso'
    reais sao recalculados depois pelo merge_google.py a partir de
    data/google_atual.json e data/google_inicial.json - por isso ficam
    como placeholder aqui.
    """
...
def aplicar_google_manual(resultado, mes_ref):
    ...
    # Indice por nome normalizado (sem acento/maiusculas) para evitar
    # que "Veranopolis" (sistema) e "Veranopolis" (arquivo manual) sejam
    # tratados como cidades diferentes.
    google_manual_normalizado = {
        normalizar_texto(chave): (chave, valor) for chave, valor in google_manual.items()
    }
