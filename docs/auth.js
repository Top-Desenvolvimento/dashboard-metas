...
const AUTH_VERSION = "AUTH_VERSAO_20260326_01";
...
style.textContent = `
  html.auth-loading, body.auth-loading { overflow: hidden !important; }
  ...
  .auth-card {
    ...
    padding: 28px; color: #eef8ff;
  }
  .auth-card h2 { font-size: 2rem; margin: 0 0 8px 0; font-weight: 900; letter-spacing: -0.03em; }
  .auth-card p { color: #7b90a6; margin: 0 0 18px 0; }
  .auth-label { display: block; margin: 12px 0 8px; font-weight: 700; }
  .auth-input {
    width: 100%; padding: 14px 16px; border-radius: 14px;
    border: 1px solid rgba(255,255,255,.08); background: rgba(255,255,255,.03);
    color: #eef8ff; outline: none; box-sizing: border-box;
  }
  .auth-actions { display: grid; grid-template-columns: 1fr 1fr; gap: 12px; margin-top: 18px; }
  .auth-btn {
    padding: 14px 16px; border: none; border-radius: 14px;
    cursor: pointer; font-weight: 800; transition: .2s ease;
  }
  .auth-btn.primary { color: #04111d; background: linear-gradient(135deg, rgba(0,232,255,.95), rgba(18,217,159,.95)); }
  .auth-btn.secondary { color: #eef8ff; background: rgba(255,255,255,.05); border: 1px solid rgba(255,255,255,.08); }
  .auth-msg { margin-top: 16px; min-height: 24px; font-weight: 700; line-height: 1.4; }
  .auth-msg.error { color: #ff4d7a; }
  .auth-msg.success { color: #12d99f; }
  .auth-note { margin-top: 12px; font-size: 12px; color: #7b90a6; word-break: break-all; }
  .auth-userbar {
    position: fixed; top: 18px; right: 24px; z-index: 999998;
    display: flex; gap: 10px; align-items: center; flex-wrap: wrap;
  }
  .auth-pill {
    padding: 12px 14px; border-radius: 14px; background: rgba(8, 20, 33, 0.92);
    border: 1px solid rgba(0, 232, 255, 0.12);
    box-shadow: 0 0 0 1px rgba(0,232,255,.05), 0 14px 28px rgba(0,0,0,.28);
    color: #eef8ff; font-weight: 700; font-family: Inter, Arial, sans-serif;
  }
  .auth-pill.btn { color: #04111d; background: linear-gradient(135deg, rgba(0,232,255,.95), rgba(18,217,159,.95)); border: none; cursor: pointer; }
...
async function isAllowed(email) {
  ...
  if (error) {
    console.error("allowed_users error:", error);
    return false;
  }
...
loginBtn.addEventListener("click", async () => {
  ...
  try {
    ...
    setMsg("Erro no login: " + (err?.message || "falha de conexão"));
  }
});
