...
def aplicar_google_em_base(base, google_mes, mes_ref):
    alterados = 0
    google_norm = {normalizar_nome(k): (k, v) for k, v in google_mes.items()}

    for cidade, info_cidade in base.items():
        chave = normalizar_nome(cidade)
        if chave not in google_norm:
            print(f"Sem Google manual para {cidade}")
            continue
        _, dados = google_norm[chave]
        indicadores = info_cidade.setdefault("indicadores", {})
        indicador = montar_indicador_google(dados)
